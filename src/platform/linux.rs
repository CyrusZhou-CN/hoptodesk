...
fn stop_subprocess() {
    let _ = run_cmds(&format!(
        r##"ps -ef | grep '/etc/hoptodesk/xorg.conf' | grep -v grep | awk '{{printf("kill -9 %d\n", $2)}}' | bash"##,
    ));
    let _ = run_cmds(&format!(
        r##"ps -ef | grep -E 'hoptodesk +--cm-no-ui' | grep -v grep | awk '{{printf("kill -9 %d\n", $2)}}' | bash"##,
    ));
}
...
mod desktop {
    ...
    impl Desktop {
        ...
        fn set_is_subprocess(&mut self) {
            ...
            let cmd = "ps -ef | grep 'hoptodesk/xorg.conf' | grep -v grep | wc -l";
...
fn switch_service(stop: bool) -> String {
    ...
    if home != "/root" && !Config::get().is_empty() {
        format!("cp -f {home}/.config/hoptodesk/HopToDesk.toml /root/.config/hoptodesk/; cp -f {home}/.config/hoptodesk/HopToDesk2.toml /root/.config/hoptodesk/;")
    } else {
        "".to_owned()
    }
}
...
pub fn uninstall_service(show_new_window: bool) -> bool {
    ...
    if !run_cmds_pkexec(&format!(
        "systemctl disable hoptodesk; systemctl stop hoptodesk; {cp}"
    )) {
...
pub fn install_service() -> bool {
    ...
    if !run_cmds_pkexec(&format!(
        "{cp} systemctl enable hoptodesk; systemctl start hoptodesk;"
    )) {
...
fn check_if_stop_service() {
    if Config::get_option("stop-service".into()) == "Y" {
        allow_err!(run_cmds(
            "systemctl disable hoptodesk; systemctl stop hoptodesk"
        ));
    }
}
...
pub fn check_autostart_config() -> ResultType<()> {
    ...
    let file = format!("{path}/hoptodesk.desktop");
